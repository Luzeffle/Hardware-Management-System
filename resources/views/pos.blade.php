<x-app-layout>
    ...
    <main x-data="posApp()" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <!-- Left: Items panel (spans 2 cols on large screens) -->
        <section class="lg:col-span-2 bg-white rounded shadow-sm p-4">
            <div class="flex items-center justify-between mb-4">
                <div class="w-full max-w-md">
                    <label class="relative block">
                        <span class="sr-only">Search</span>
                        <input x-model="search" placeholder="Search here" class="placeholder-gray-400 bg-gray-100 border border-gray-200 rounded px-3 py-2 w-full focus:outline-none focus:ring-2 focus:ring-indigo-200" />
                    </label>
                </div>
            ...
            <div class="border border-gray-200 rounded">
                <div class="px-4 py-3 bg-gray-100 border-b border-gray-200 font-medium">Items</div>
                <div class="overflow-auto" style="max-height:48vh">
        ...
        <aside class="bg-white rounded shadow-sm p-6">
            <h3 class="text-2xl font-semibold mb-4 text-center">Order</h3>
		...
		<script>
			function posApp(){
				return {
					...
					nextUid: 1,
					addItemDemo(){
						this.order.push({uid: this.nextUid, qty: 1, price: 120.00, name: 'Demo Product ' + this.nextUid});
						this.nextUid++;
					},
					removeItem(uid){ this.order = this.order.filter(i => i.uid !== uid) },
					clearOrder(){ this.order = [] },
					getTotal(){ return this.order.reduce((s,i)=>s + (i.qty * i.price), 0) },
					formatPrice(v){ return Number(v).toLocaleString(undefined, {minimumFractionDigits:2, maximumFractionDigits:2}) }
				}
			}
		</script>
	@endpush
